...
const UploadPage = () => {
 ...
 if (!file) {
  openModal("error", "Upload Failed", "Please select a file to upload.");
  setUploading(false);
  return;
 }

 if (!title.trim() || !subject || !year) {
  openModal("error", "Validation Error", "Please fill in all required fields (Title, Subject, Year).");
  setUploading(false);
  return;
 }
 ...
 // Simulate upload
 setTimeout(() => {
  openModal("success", "Upload Successful", "Resource uploaded successfully!");
  setTitle("");
  setDescription("");
  setSubject("");
  setCourse("");
 ...
 <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-orange-50 dark:from-onyx dark:via-charcoal dark:to-onyx text-gray-900 dark:text-gray-100 transition-all duration-300">
  ...
  {/* The CustomWarningModal component is rendered here */}
  <CustomWarningModal
  isOpen={modal.isOpen}
  onClose={closeModal}
  type={modal.type}
