...
const ResourcesSection = ({ searchQuery, filterType, filterCourse, sortBy, setSortBy, showModal }) => {
    ...
    <section className="px-4 py-8 max-w-full  mx-auto bg-gray-50 dark:bg-onyx min-h-screen  shadow-glow-sm border border-gray-200 dark:border-onyx transition-all duration-300 animate-fade-in hover:bg-gray-100 ">
        ...
        {loading && (
            <div className="text-center py-12 flex flex-col items-center justify-center text-gray-600 dark:text-platinum font-poppins">
                <FontAwesomeIcon icon={faSpinner} spin size="2x" className="mb-4 text-amber-600 dark:text-amber-200" />
        ...
        <div className="max-w-6xl  grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 mx-auto gap-8 ">
            {displayedResources.map(resource => (
                <ResourceCard key={resource._id} resource={resource} showModal={showModal} />
            ))}
        </div>
    </section>
);
