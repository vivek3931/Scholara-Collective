...
const Hero = () => {
  ...
  useEffect(() => {
    ...
    const createParticleAnimation = () => {
      if (heroRef.current && particlesRef.current.length > 0) {
        ...
        particlesRef.current.forEach((particle, index) => {
          ...
          const size = particleType === 0 ? gsap.utils.random(2, 4) : 
                       particleType === 1 ? gsap.utils.random(4, 6) : 
                       gsap.utils.random(6, 10);
          
          const opacity = particleType === 0 ? gsap.utils.random(0.3, 0.6) : 
                          particleType === 1 ? gsap.utils.random(0.4, 0.8) : 
                          gsap.utils.random(0.6, 1);
          
          const blur = particleType === 0 ? 0 : 
                       particleType === 1 ? 2 : 
                       4;
    ...
    const animateParticle = (particle, {
      ...
      duration, size, opacity, blur,
      delay, ease, shape, rotation = 0
    }) => {
      ...
      const midX = centerX + Math.cos(angle) * distance * gsap.utils.random(0.3, 0.7);
      const midY = centerY + Math.sin(angle) * distance * gsap.utils.random(0.3, 0.7);
        ...
        onComplete: () => {
          ...
          gsap.to(particle, {
            ...
            onComplete: () => {
              ...
              const newDistance = gsap.utils.random(minDistance * 0.8, maxDistance * 1.2);
              animateParticle(particle, {
                centerX, centerY,
                angle: newAngle,
                minDistance, maxDistance: newDistance,
  ...
  const particles = Array.from({ length: 50 }, (_, index) => {
    ...
    const size = type === 0 ? Math.random() * 2 + 2 : 
                 type === 1 ? Math.random() * 4 + 3 : 
                 Math.random() * 6 + 4;
  ...
  return (
    ...
    >
      ...
      >
        Unlock Your <span className="bg-gradient-to-r from-orange-500 via-yellow-500 to-orange-600 bg-clip-text text-transparent">Academic Potential</span>
      </h1>
      <p
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-center animate-fade-in-up delay-400 max-w-lg sm:max-w-none mx-auto">
        ...
        >
          <FontAwesomeIcon icon={faCloudUploadAlt} className="text-lg sm:text-xl" /> 
