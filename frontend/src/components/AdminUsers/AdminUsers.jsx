      ...
      <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block align-bottom bg-white  dark:bg-charcoal rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
          ...
          <div className="bg-gray-50 dark:bg-charcoal/40  px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
            <button
              type="button"
              className={`w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 text-base font-medium text-white sm:ml-3 sm:w-auto sm:text-sm transition-colors duration-200 ${confirmColor === 'red' ? 'bg-red-600 hover:bg-red-700' : 'bg-amber-600 hover:bg-amber-700'}`}
              onClick={onConfirm}
            >
              {confirmText}
            </button>
...
const AdminUsers = () => {
  ...
  const [modalContent, setModalContent] = useState({ title: '', message: '', onConfirm: () => {}, confirmText: '', cancelText: 'Cancel', confirmColor: 'amber' });
  ...
  useEffect(() => {
    fetchUsers();
  }, [fetchUsers]); // Correct dependency array, including the memoized function

  // Centralized function to handle confirmation from the modal
    ...
    setModalContent({
      ...
      onConfirm: async () => {
        try {
          ...
          // Instead of an alert, we can show a temporary success message
          alert('User promoted to admin successfully!'); // Replace this with a proper toast notification or similar
          fetchUsers(); // Refresh the list
        } catch (err) {
