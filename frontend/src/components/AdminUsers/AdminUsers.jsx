  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <div
          className="fixed inset-0 bg-gray-500 dark:bg-charcoal/20 backdrop-blur-sm bg-opacity-75 transition-opacity"
          aria-hidden="true"
        ></div>
        <span
...
const AdminUsers = () => {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    setError(null);
    try {
      // FIX: Correctly pass positional arguments to getUsers function
      ...
      setError(
        "Failed to load users. Please ensure you are logged in with appropriate permissions."
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleToggleStatus = (userId, currentStatus) => {
    ...
    setModalContent({
      ...
      onConfirm: async () => {
        try {
          // FIX: Use the dedicated api.toggleUserStatus function
  ...
  const handleDeleteUser = (userId) => {
    setModalContent({
      title: "Delete User",
      message:
        "Are you sure you want to delete this user and ALL their associated data (resources, comments, ratings)? This action is irreversible.",
      onConfirm: async () => {
        try {
          // FIX: Use the dedicated api.deleteUser function
  ...
  const handlePromoteUser = (userId) => {
    setModalContent({
      ...
      message:
        "Are you sure you want to promote this user to admin? This action typically requires Super Admin privileges.",
      onConfirm: async () => {
        try {
          // FIX: Use the dedicated api.promoteUserToAdmin function
          const response = await api.promoteUserToAdmin(userId);
          ...
          const errorMessage =
            err.response?.data?.msg || err.message || "Failed to promote user";
          setError(errorMessage);
        }
      },
      confirmText: "Promote",
  ...
  return (
    <div className="p-6 md:p-8 bg-white dark:bg-onyx rounded-2xl shadow-lg border border-gray-200 dark:border-charcoal transition-all duration-300">
      ...
      <ConfirmationModal
        ...
        onConfirm={modalContent.onConfirm ? handleModalConfirm : null}
      ...
      {/* Error display */}
      {error && (
        <div className="mb-6 bg-red-50 dark:bg-red-900/20 border-l-4 border-red-500 text-red-700 dark:text-red-300 p-4 rounded-lg shadow-sm">
        ...
        <>
          {users.length === 0 ? (
            ...
            <div className="space-y-4">
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-700 dark:text-gray-200">
                        ↑{user.stats?.uploadCount || user.uploadCount || 0} ↓
                        {user.stats?.downloadCount || user.downloadCount || 0}
                      </p>
                    </div>
              ...
              {/* Pagination */}
              {totalPages > 1 && (
                <div className="flex items-center justify-between px-4 py-3 bg-white dark:bg-onyx border-t border-gray-200 dark:border-charcoal sm:px-6">
                  <div className="flex flex-1 justify-between sm:hidden">
                    <button
                      onClick={() => handlePageChange(currentPage - 1)}
                      disabled={currentPage === 1}
                      className="relative inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-200 bg-white dark:bg-onyx border border-gray-300 dark:border-charcoal rounded-md hover:bg-gray-50 dark:hover:bg-onyx-700 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Previous
                    </button>
                    <button
                      ...
                      className="relative ml-3 inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-200 bg-white dark:bg-onyx border border-gray-300 dark:border-charcoal rounded-md hover:bg-gray-50 dark:hover:bg-onyx-700 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Next
                    </button>
                  </div>
                    ...
                    <div>
                      ...
                      >
                        <button
                          onClick={() => handlePageChange(currentPage - 1)}
                          disabled={currentPage === 1}
                          className="relative inline-flex items-center px-2 py-2 rounded-l-md border border-gray-300 dark:border-charcoal bg-white dark:bg-onyx text-sm font-medium text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-onyx-700 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            ...
                            return (
                              <button
                                ...
                                className={`relative inline-flex items-center px-4 py-2 border text-sm font-medium ${
                                  pageNum === currentPage
                                    ? "z-10 bg-amber-50 dark:bg-amber-900/20 border-amber-500 dark:border-amber-400 text-amber-600 dark:text-amber-200"
                                    : "bg-white dark:bg-onyx border-gray-300 dark:border-charcoal text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-onyx-700"
                                }`}
                              >
                                {pageNum}
                              </button>
                        ...
                        <button
                          onClick={() => handlePageChange(currentPage + 1)}
                          disabled={currentPage === totalPages}
                          className="relative inline-flex items-center px-2 py-2 rounded-r-md border border-gray-300 dark:border-charcoal bg-white dark:bg-onyx text-sm font-medium text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-onyx-700 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
