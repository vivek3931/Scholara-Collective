...
import { LineChart, Pie, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, PieChart as RechartsPieChart, Cell, BarChart, Bar, Area, AreaChart } from 'recharts';
...
const AdminDashboard = () => {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        if (err.response?.status === 403) {
          ...
          setError(`Failed to load dashboard data: ${err.response?.data?.msg || err.message}`);
        }
      } finally {
        setLoading(false);
      }
  ...
  if (error) {
    return (
      <div className="min-h-[calc(100vh-64px)] flex items-center justify-center bg-gray-50 dark:bg-onyx">
        <div className="text-center p-8 bg-white dark:bg-gray-800 rounded-2xl shadow-glow-sm border border-gray-200 dark:border-onyx">
          <AlertTriangle className="mx-auto h-16 w-16 text-amber-500 mb-4 dark:text-amber-300" />
          <h3 className="text-xl font-bold text-red-700 dark:text-amber-300 mb-2 font-poppins">Dashboard Error</h3>
          <p className="text-red-600 dark:text-amber-400 font-poppins">{error}</p>
        </div>
      </div>
    );
  }
  ...
  const StatCard = ({ title, value, icon: Icon, growth, color, subtitle }) => (
    <div className="bg-white dark:bg-onyx/80 rounded-2xl p-6 shadow-glow-sm border border-gray-200 dark:border-onyx hover:shadow-glow-sm transition-all duration-300 hover:scale-[1.02]">
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-onyx lg:p-6 p-2 transition-all duration-300 animate-fade-in hover:bg-gray-100">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          {/* Weekly Growth Chart */}
          <div className="xl:col-span-2 bg-white dark:bg-onyx/80 rounded-2xl p-6 shadow-glow-sm border border-gray-200 dark:border-onyx">
          ...
          <div className="bg-white dark:bg-onyx/80 rounded-2xl p-6 shadow-glow-sm border border-gray-200 dark:border-onyx">
            <div className="flex items-center justify-between mb-6">
              <div>
